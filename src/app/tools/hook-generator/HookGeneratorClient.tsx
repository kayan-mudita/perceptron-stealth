        ...
        <div className="max-w-3xl mx-auto text-center">
          <FadeIn>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-violet-500/10 border border-violet-500/20 mb-6">
              ...
              <span className="text-p3 text-violet-400/80 font-medium">Free tool</span>
            </div>
            ...
            <p className="text-title text-white/35 max-w-xl mx-auto leading-relaxed font-light">
              {totalCount}+ proven first-line hooks for short-form video. Pick your industry and start scripting.
            </p>
          </FadeIn>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
            <div className="flex flex-wrap gap-2">
              {(Object.keys(LABELS) as Industry[]).map((key) => (
                <button
                  ...
                  className={`text-p3 px-4 py-2 rounded-lg border transition-all ${
                    industry === key
            ...
            <button
              ...
              className="inline-flex items-center gap-2 text-p3 px-4 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white/60 hover:text-white hover:border-white/[0.16] transition-all"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {shown.map((hook, i) => (
              <button
                key={`${industry}-${seed}-${i}`}
                onClick={() => copy(hook)}
                className="group text-left p-4 rounded-xl card-hairline hover:border-white/[0.14] transition-all flex items-start justify-between gap-3"
              >
                <span className="text-p2 text-white/75 leading-relaxed">{hook}</span>
                {copied === hook ? (
                  <Check className="w-4 h-4 mt-1 text-emerald-400 flex-shrink-0" />
                ) : (
                  <Copy className="w-4 h-4 mt-1 text-white/20 group-hover:text-white/60 flex-shrink-0 transition-colors" />
                )}
              </button>
            ))}
          </div>
          ...
          <div className="mt-8 p-5 rounded-xl card-hairline">
            <h2 className="text-p2 font-semibold text-white/80 mb-2">How to use these hooks</h2>
            <p className="text-p3 text-white/40 leading-relaxed">
              The first 3 seconds of a short-form video decide whether someone keeps watching. These hooks
              are pattern-tested across LinkedIn, TikTok, and Reels. Pair one with a single concrete claim
              and one CTA — that's the entire script.
            </p>
          </div>
        </div>
      </section>
